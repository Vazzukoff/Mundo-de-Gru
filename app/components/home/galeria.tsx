...
// Cargar Carousel solo en cliente (IMPORTANTE)
const Carousel = dynamic(() => import("@/utils/carousel"), { ssr: false });
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        ...
        if (entry.target === headerRef.current) {
          setTimeout(() => setIsVisible((p) => ({ ...p, header: true })), 100);
        }

        if (entry.target === dividerRef.current) {
          setTimeout(() => setIsVisible((p) => ({ ...p, divider: true })), 300);
        }

        if (entry.target === descriptionRef.current) {
          setTimeout(() => setIsVisible((p) => ({ ...p, description: true })), 500);
        }

        if (entry.target === carouselRef.current) {
          setTimeout(() => setIsVisible((p) => ({ ...p, carousel: true })), 700);
        }
      });
    }, observerOptions);

    [headerRef, dividerRef, descriptionRef, carouselRef].forEach((ref) => {
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <header className="text-center mb-12">
          ...
          >
            Conoce nuestras instalaciones de guardería, espacios de estimulación
            temprana y el ambiente de aprendizaje que ofrecemos en nuestro nido
          </p>
        </header>
        ...
        <div className="flex justify-center items-center">
          <div
            ...
            className={`bg-gray-50 rounded-2xl p-2 sm:p-4 md:p-8 shadow-lg border border-gray-100 w-full max-w-5xl transition-all duration-700 ${
              isVisible.carousel ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
            }`}
            role="region"
